...
do
awk -f dvdl_direct.awk ../$a/$a'_us.out' > vd_$a'_temp1.txt'
awk '!a[$1]++' vd_$a'_temp1.txt' > vd_$a'_temp2.txt'
awk '{ sum += $2} END { printf("%5.1f", sum/NR)}' vd_$a'_temp2.txt' > vd_$a'_dvdl.txt'
done
...
awk '{charge=-$1; printf ("%5.1f", charge)}' TOTAL_dvdl.txt > 1_VDW.txt
